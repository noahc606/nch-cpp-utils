...
std::map<std::string, SDL_Surface*> AtlasImage::collectFromPaths(const std::vector<std::vector<std::string>>& objCollections, const std::vector<std::string>& collectionPrefixes, bool jsonFiles) {
	...
	for(size_t i = 0; i<objCollections.size(); i++) {
		...
		for(std::string obj : objPaths) {
			...
			if(jsonFiles && fp.getExtension()=="json") {
			...
			fp.getParentDirName();
			ret.insert({nch::cat(prefix, fp.getObjectName(false)), surf});
		}
	}
...
std::map<std::string, SDL_Surface*> AtlasImage::collectFromDirs(const std::vector<std::string>& dirPaths, const std::vector<std::string>& prefixes, bool jsonFiles) {
	...
	return collectFromPaths(manyDirConts, prefixes, jsonFiles);
}
std::map<std::string, SDL_Surface*> AtlasImage::collectFromDir(const std::string& dirPath, const std::string& prefix, bool jsonFiles)
{
	...
	return collectFromPaths({dirConts}, {prefix}, jsonFiles);
}
...
SDL_Surface* AtlasImage::buildSurfaceFromJSON(const std::string& jsonPath)
{
	...
	if(w==0 || h==0) {
		Log::errorv(__PRETTY_FUNCTION__, "skipping", "JSON \"%s\": no valid \"img\" found to determine dimensions", jsonPath.c_str());
		return nullptr;
	}
	...
	for(auto& elem : elems) {
		...
		if(elem.contains("colormod")) {
			...
			if(!cm.is_array() || (cm.size()!=3 && cm.size()!=4)) {
				Log::errorv(__PRETTY_FUNCTION__, "skipping file", "JSON \"%s\": \"colormod\" must be an array of 3 or 4 ints", jsonPath.c_str());
